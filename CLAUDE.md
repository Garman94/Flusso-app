...
Documentazione tecnica completa per Claude Code. Aggiornata al: 2026-09-10. Ultima modifica: 2026-09-15 (Accantonamenti collegabili a un salvadanaio specifico; categoria "Accantonamenti" ora trattata come trasferimento interno — esclusa da entrate/spese come Spostamenti/Salvadanaio).
...
> **2026-09-15 — trasferimenti centralizzati**: `TRANSFER_CATEGORY_NAMES`/`isTransferCategory` (`lib/calculations.ts`) sostituiscono i `TRANSFER_CATS` locali duplicati in `balance-hero-card.tsx`, `month-report-modal.tsx` e `dashboard-client.tsx` (anche il breakdown per categoria della dashboard/report li usa, quindi ora esclude anche `Accantonamenti`). Aggiunta "Accantonamenti" all'insieme: le transazioni con quella categoria erano l'unico trasferimento non escluso — coprivano anche i prelievi reali dal salvadanaio, perché l'import Excel già li auto-categorizza come "Accantonamenti" per parola chiave (`import-excel-modal.tsx`). Per i bonifici occasionali senza categoria naturale (es. un prestito restituito da terzi) non è stato aggiunto un flag dedicato: si usa la categoria "Spostamenti" già esistente e già esclusa. `app/dashboard/estimate-and-savings-card.tsx` (codice morto, vedi nota 2026-09-14 sopra) non è stato aggiornato: mantiene ancora il vecchio `TRANSFER_CATS` locale senza "Accantonamenti", irrilevante finché resta non importato.
...
>
> Le transazioni con categoria "Accantonamenti" 🏦 sono ora trattate come trasferimento interno (vedi `isTransferCategory` sotto) e non contano più come spesa generica in dashboard. Nella lista voci compare comunque una card "💰 Accantonato da transazioni questo mese" con la somma di quelle transazioni nel periodo corrente (solo notifica informativa — nessun collegamento automatico a una voce specifica).
...
| `isTransferCategory` (+ `TRANSFER_CATEGORY_NAMES`) | Trasferimenti interni (`spostamenti`/`salvadanaio`/`accantonamenti`) da escludere da entrate/spese/breakdown — usata da dashboard, month-report e hero card |
