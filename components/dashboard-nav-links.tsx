...
function useNavTourDots() {
  ...
  return { unseenMap, isActive, handleDot };
}

export function DashboardNavLinks({ isAdmin }: Props) {
  const { unseenMap, isActive, handleDot } = useNavTourDots();
...
// Renderizzato FUORI dalla top bar (che ha backdrop-blur e quindi crea un
// containing block per i figli position:fixed, facendo apparire questo nav
// in alto invece che in basso se annidato lì dentro).
