-- Migration 037: la demo mostra Flusso com'è oggi.
-- Prima reseed_demo creava spese ricorrenti col vecchio sistema (che ora finirebbero in
-- "Spese fisse da sistemare") e riempiva budget_items, una tabella che l'interfaccia non
-- usa più: la vetrina mostrava "hai superato le spese previste" e voci da sistemare.
-- Ora: budget per categoria, due accantonamenti, e le entrate di entrambi i componenti.
-- Tocca solo i dati dell'utente demo@flussoapp.it.
...
begin
  ...
  -- Budget per categoria (Pianifica → Budget): è quello che la dashboard usa per le spese previste
  insert into category_budgets (user_id, category_id, monthly_budget) values
    (demo_id, cat_casa,            800.00),
    (demo_id, cat_alimentari,      320.00),
    (demo_id, cat_trasporti,       150.00),
    (demo_id, cat_bollette,        150.00),
    (demo_id, cat_ristoranti,      120.00),
    (demo_id, cat_intrattenimento,  40.00),
    (demo_id, cat_salute,           40.00),
    (demo_id, cat_hobby,            40.00);

  -- Accantonamenti (spese annuali): affitto, bollette e abbonamenti stanno nel Budget
