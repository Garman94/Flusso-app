-- Rollback della migration 037: rimette la reseed_demo() com'era in produzione prima
-- (copiata dal database il 2026-09-24 con pg_get_functiondef) e ricarica i dati demo.
-- Tocca solo i dati dell'utente demo@flussoapp.it.
-- Uso, solo se serve tornare indietro:
--   npx --no-install supabase db query --linked -f supabase/rollback/037_demo_pianifica_down.sql
...
-- La vecchia funzione non svuota category_budgets: senza questa riga resterebbero i budget della 037.
delete from category_budgets where user_id = public.demo_user_id();
